...
- [x] Update restaurant charge-to-room to post to `GuestFolio` once explicit folios exist.
...
Status as of 2026-06-15: restaurant charge-to-room now posts to the explicit
`GuestFolio` when one is available, stores the linked `FolioCharge`,
`FolioLineItem`, and `OrderPayment` in one transaction, and records those ids in
...
Status as of 2026-06-23: checkout blocks final close when the previewed amount
due is greater than zero. Authorized-balance/corporate exceptions are still
reserved for the later company-account workflow, so the current production path
requires ordinary settlement before checkout.

Status as of 2026-06-23: open folios can record confirmed settlement payments
